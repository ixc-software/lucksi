...
void Moc::parse()
{
    ...
    while (hasNext()) {
        ...
        if (parseClassHead(&def)) {
			...
			QString classNamespaces;
            for (int i = namespaceList.size() - 1; i >= 0; --i)
			{
                if (inNamespace(&namespaceList.at(i)))
				{
					classNamespaces.prepend(
						QString::fromLocal8Bit(namespaceList.at(i).name.constData()) + 
            ...
            extentionMoc.Append(def, classNamespaces);// moc_ext: append proxy interface to list
...
void Moc::generate(FILE *out)
{
	...
	extentionMoc.Generate(out);// moc_ext: generate proxy interfaces
