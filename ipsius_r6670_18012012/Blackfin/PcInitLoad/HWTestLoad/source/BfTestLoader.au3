...
#include-once
	...
	$FlashProjFileMain = "HWTest.bin"
	$FlashProjFileEcho = "HWTestEcho.bin"	
...
;	MsgBox(0, "Отладка", "Запись проекта может занять до 15-ти минут",1)			
	...
	ControlSend($FlashUartTitle, "", "[CLASSNN:TEdit3]", "{HOME}" & "+{END}" & "{DEL}")	
;	ControlSend($FlashUartTitle, "", "[CLASSNN:TEdit3]", $EchoProgOffset)
	ControlSetText($FlashUartTitle, "", "[CLASSNN:TEdit3]", $EchoProgOffset)
	ControlClick($FlashUartTitle, "", "[CLASSNN:TButton5]")					
	WinWaitCheck("Открыть", "окно открытия файла не найдено")
;	ControlSend("Открыть", "", "[CLASSNN:Edit1]", @ScriptDir & "\" & "UsedExe" & "\" & $FlashProjFileEcho & "{ENTER}")
	ControlSetText("Открыть", "", "[CLASSNN:Edit1]", @ScriptDir & "\" & "UsedExe" & "\" & $FlashProjFileEcho)
	ControlSend("Открыть", "", "[CLASSNN:Edit1]", "{ENTER}")
	WinActivate($FlashUartTitle)	
	ControlClick($FlashUartTitle, "", "[CLASSNN:TButton4]")				
	
	$i = $LoadProjTimeoutSec
	While (StringInStr(ControlGetText($FlashUartTitle, "", "[CLASSNN:TMemo1]"), $EndOfLoad, 0, 2) < 1)
		Sleep(1000)
		$i = $i - 1
		if ($i == 0) Then
			MsgBox(0, "Отладка", "Превышен таймаут записи файла")	
